...
| M6 | Inference API | ⚪ Not started | No `src/` or `app/`. `preprocessing.pkl` covers only the last two steps — it expects the **engineered 27-column frame**, not a raw record (verified: `selector.n_features_in_ = 27`, raw input raises on feature names). See the risk below for what is still unserializable. |
...
|------|--------|--------------|-----------|
| **Half the inference path is not serializable as written.** `preprocessing.pkl` resumes from the engineered frame. Ahead of it sit the column drops, the `LabelEncoder` for `Gender` (fitted, never saved), the 24 feature formulas, and cell 11's zero-replacement — which fills from `df[col].mean()` **recomputed from whatever frame is loaded** | M6 cannot score a single raw record. The mean-fill is undefined for one row, and on the training frame it is computed across all 3000 rows including test — a second, unlogged leak | Move preprocessing into one `Pipeline`/`ColumnTransformer` fitted on train and serialized whole, with the zero-fill as a fitted `SimpleImputer` rather than an inline `.mean()` | 2026-08-27 |
